...
if (isset($_POST['order_btn'])) {
    ...
    if ($cart_total == 0) {
        $message[] = 'Ваша корзина пусто';
    } else {
        if (mysqli_num_rows($order_query) > 0) {
            $message[] = 'Заказ уже сделан!';
        } else {
            mysqli_query($conn,"INSERT INTO `orders`(user_id, name, number, email, method, address, 
            total_products, total_price, placed_on) VALUES('$user_id', '$name', '$number', '$email', 
            '$method', '$address', '$total_products', '$cart_total', '$placed_on')") or die('query failed');
...
<div class="heading">
    <h3>Оплата</h3>
    <!-- <p><a href="home.php">Главный</a> / оплата </p> -->
</div>
...
<section class="display-order">
    ...
    if (mysqli_num_rows($select_cart) > 0) {
        while ($fetch_cart = mysqli_fetch_assoc($select_cart)) {
            ...
            ?>
            <p> <?php
                echo $fetch_cart['name']; ?> <span>(<?php
                    echo '$' . $fetch_cart['price'] . '/-' . ' x ' . $fetch_cart['quantity']; ?>)</span>
            </p>
            <?php
        }
    } else {
        echo '<p class="empty">Ваша корзина пусто</p>';
    }
    ?>
    <div class="grand-total"> Общая сумма : <span>$<?php
    ...
    <form action="" method="post">
        ...
        <div class="flex">
            ...
            <div class="inputBox">
                ...
                <select name="method">
                    <option value="Оплата наличнимы при доставке">Оплата при доставке</option>
