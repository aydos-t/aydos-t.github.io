...
if (isset($_POST['add_product'])) {
    ...
    if (mysqli_num_rows($select_product_name) > 0) {
        $message[] = 'Это книга уже добавлена!';
    } else {
        ...
        if ($add_product_query) {
            ...
            $message[] = 'Книга не может быть добавлена!';
        }
    }
}
...
<section class="add-products">
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <input type="number" min="0" name="price" class="box" placeholder="Цена книги" required>
        ...
        if (mysqli_num_rows($select_products) > 0) {
            while ($fetch_products = mysqli_fetch_assoc($select_products)) {
                ...
                <div class="box">
                    ...
                    <a href="admin_products.php?update=<?php 
                        echo $fetch_products['id']; ?>" class="option-btn">Обновить книгу
                    </a>
                    <a href="admin_products.php?delete=<?php
                        echo $fetch_products['id']; ?>" 
                        class="delete-btn" onclick="return confirm('Удалить эту книгу?');">Удалить книгу
                    </a>
                </div>
                <?php
            }
        } else {
            echo '<p class="empty">Книга ещё не добавлена!</p>';
        }
        ?>
    </div>
</section>
        ...
        if (mysqli_num_rows($update_query) > 0) {
            while ($fetch_update = mysqli_fetch_assoc($update_query)) {
                ...
                <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <input type="text" name="update_name" value="<?php 
                       echo $fetch_update['name']; ?>" class="box" required placeholder="Введите название книги">
                    <input type="number" name="update_price" value="<?php
                        echo $fetch_update['price']; ?>" class="box" min="0" required placeholder="Цена книги">
